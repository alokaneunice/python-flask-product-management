    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute("INSERT INTO products (name, price, quantity, barcode) VALUES (?,?,?,?)",(nm, price, quantity, barcode))
...
def editrec():
    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute("UPDATE products SET name='"+nm+"', price='"+price+"', quantity='"+quantity+"', barcode='"+barcode+"' WHERE rowid="+rowid)

                con.commit()
                msg = "Record successfully edited in the database"
        except:
            con.rollback()
            msg = "Error in the Edit: UPDATE products SET name="+nm+", price="+price+", quantity="+quantity+", barcode="+barcode+" WHERE rowid="+rowid
